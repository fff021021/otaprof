...
export default function ProfileView() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      // ----------------------------------------
      // 旧URL方式
      // /profile/view?d=xxxxx
      //
      // 以前作成した共有URLも引き続き閲覧できる
      // ----------------------------------------
      if (encoded) {
  ...
  const handleCopyUrl = async () => {
    ...
    try {
      // Firebaseに保存して短いID付きURLを生成
      const url = await generateProfileShareUrl(
        profileWithColor
      );
  ...
  // ========================================
  const snsLabel =
    SNS_STYLE_OPTIONS.find(
      (o) =>
        o.value === profile?.values?.snsStyle
    )?.label ?? '';
            ...
            <div className="values-axis">
              {VALUES_QUESTIONS.map((q) => {
                ...
                return (
                  <div
                    key={q.key}
                    className="values-axis-row"
                  >
                    <span
                      className="values-axis-label"
                      style={{
                        fontSize:
                          '0.8125rem',
                      }}
        ...
        {isSharedView && (
          ...
          >
            ...
            <Link
              to={
                searchParams.get('id')
                  ? `/match?targetId=${searchParams.get('id')}`
                  : `/match?target=${searchParams.get('d')}`
              }
          ...
          <div className="share-box">
            ...
            <div className="share-url-row">
              <input
                ...
                placeholder="「共有URLをコピー」を押すと生成されます"
              />
