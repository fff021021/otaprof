...
export default function FukyoView() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      // ----------------------------------------
      if (id) {
        const data = await loadFukyoData(id);
      ...
      if (profileSaved) {
        try {
          setAuthorProfile(JSON.parse(profileSaved));
        } catch {
          // noop
        }
      }
  ...
  const handleCopyUrl = async () => {
    ...
    try {
      // Firebaseに保存して短いID付きURLを作成
      const url = await generateFukyoShareUrl(payload);
